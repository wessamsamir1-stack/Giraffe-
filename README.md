...
### على المتصفح — Running on Chrome

لو مافيش محاكي أندرويد ولا موبايل موصّل، شغّله على المتصفح:

```bash
flutter run -d chrome
```

المنصات الجاهزة في الريبو:

`android` · `ios` · `web`

والويب للتجربة السريعة بس — المنتج نفسه موبايل.

التوجيه على الويب بالـ hash، يعني الشاشات بتتفتح كده:

`http://localhost:PORT/#/moderation`
...
```bash
# على سوبابيز — الربط مرة واحدة، والدفع بعد كده
...
```
supabase/
├── config.toml   إصدار Postgres والمخططات المكشوفة وسياسة الدوال
...
مش في الريبو عن قصد — الربط بيتخزن محلياً في

`supabase/.temp/`

عشان الريبو مايتقفلش على مشروع سحابي واحد.
